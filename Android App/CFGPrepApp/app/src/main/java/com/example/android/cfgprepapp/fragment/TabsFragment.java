...
public class TabsFragment extends Fragment implements View.OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		adapter = new TabsFragment.MyPagerAdapter(getActivity().getSupportFragmentManager());
	...
	public class MyPagerAdapter extends FragmentPagerAdapter {
		...
		public Fragment getItem(int position) {
			if(position == 0) {
				return TabForumFragment.newInstance(position);
			}
			else if(position == 1) {
				return TabShopFragment.newInstance(position);
			}
			else{
				return TabUserDetailFragment.newInstance(position);
			}
		}
	}
